    ...
    public partial class Log_Customer : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LoadPaymentsData()
        {
            try
            {
                id = Convert.ToInt32(txt_id.Text);
                db = new DBDMMEntities2();
                gridControl2.DataSource = db.PyamnetCustomers.Where(x => x.ID_Supplier == id).ToList();
            }
            catch { }
        ...
        private void AddPayemet()
        {
            try
            {
                ...
                var SupplierName = txt_name.Text;
                var tbpayment = new PyamnetCustomer
                {
        ...
        private void EditPayemet()
        {
            try
            {
                ...
                if (id > 0)
                {
                    ...
                    db.Set<PyamnetCustomer>().AddOrUpdate(tbpayment);
                    db.SaveChanges();
                }
                else
                {
        ...
        private void btn_paymentdelete_Click(object sender, EventArgs e)
        {
            ...
            if (rs == DialogResult.Yes)
            {
                try
                {
                    ...
                    {
                        db = new DBDMMEntities2();
                        tbpayment = db.PyamnetCustomers.Where(x => x.ID == id).FirstOrDefault();
        ...
        private void LogPaymentClearData()
        {
            try
            {
                id = Convert.ToInt16(txt_id.Text);
                db = new DBDMMEntities2();
                var Debit_List = db.PyamnetCustomers.Select(x => x.ID).ToArray();

                for (int i = 0; i < Debit_List.Length; i++)
                {
                    tbpayment = db.PyamnetCustomers.Where(x => x.ID_Supplier == id).FirstOrDefault();
                    db.Entry(tbpayment).State = EntityState.Deleted;
                    db.SaveChanges();
                }
